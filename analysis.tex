...
I tillegg viser undersøkelser gjort av Adobe at nesten 8 av 10 forbrukere vil slutte å engasjere seg i innhold, om det ikke er godt nok tilpasset deres enhet \cite{stark2015frf}.
...
\label{sec:analysis-security}
 ...
 Krypering er en prosess som gjør om data fra et leselig format til et uleslig format. For å kunne lese kryptert data trenger man nøkkelen/hemmeligheten som ble brukt til å kryptere dataene. Kryptering brukes for å forhindre at tredjeparter kan lese data som kommer på avveie.\cite[s.117-118]{NattTomHeine2015Datasikkerhet}
 
 \subsubsection{Passord hashing og salting}
...
Transport Layer Security er en kryptografisk protokoll. Denne er viktig for å sikre informasjon som overføres gjennom nettverk. TLS brukes i forskjellige protokoller som HTTPS, FTPS, VoIP og VPN.
...
SSL (Secure Socket Layer) er en utdatert forgjenger til TLS. \cite{thomas2000ssl}
 
\subsubsection{HTTPS}
\label{sec:analysis-security-https}
HTTPS (Hypertext Transfer Protocol Secure) er en sikker versjon av HTTP-protokollen som overfører data på nett. HTTPS-protokollen krever at det opprettes en kryptert kanal mellom nettleseren og webserveren, slik at overføring mellom disse blir sikker.\cite{rfc26161999hypertext}

\subsubsection{CSRF}
\label{sec:analysis-security-csrf}
Cross site request forgery (CSRF) er angrep som tvinger brukere til å utføre uønskede handlinger på en nettside der brukeren er logget inn, via en annen nettside \cite{lin2009threat}. For eksempel: hvis brukeren er logget inn på Facebook.com, og går til Twitter.com. Da kan Twitter laste inn et bilde som dette: \lstinline{<img src="https://facebook.com/delete-my-account">}. Dette vil da sende en forespørsel til den linken, som kan føre til sletting av konto. Om Facebook ikke har implementert mottiltak for slike angrep, vil denne forespørselen tolkes som om den kommer direkte fra brukeren.

\subsubsection{XSS}
\label{sec:analysis-security-xss}
Escape brukerinput.
PHP sin htmlspecialchars

\subsubsection{SQL injection}
\label{sec:analysis-security-sql-injection}
Valider input. Aldri stol på klienten. Escape brukerinput.
Bruk pepared statements

\subsubsection{MySQL}
\label{sec:analysis-security-mysql}
Bruk forskjellig brukernavn og passord på alle databaser.
Skru av remote tilgang.
Ikke gi en bruker tilgang til flere databaser.

\subsubsection{phpMyAdmin}
\label{sec:analysis-security-phpmyadmin}
Ikke bruk /phpmyadmin i linken. Gjør om til f.eks /pma, eller enda bedre; noe mindre åpenbart.
Skift brukernavn vekk fra standard "root".
Skru på HTTP Auth.

\subsubsection{Web-server med SSH tilkobling}
\label{sec:analysis-security-web-server-ssh}
Skru av remote root login og lag en egen bruker.
Skru av passord login
Bruk public/private key pair til login. Helst med passord
Sett maks login attempts.
Ban de som prøver for mye i en dag, så en uke, så en måned.
Skift vekk fra standard port "22" til f.eks. 44
Sett opp brannmur. Blokker alt bortsett fra SSH porten man har satt, 80 (http) og 443 (https)
Web server Bruk 755 på mapper og 644 på filer.

\section{Analyse av opprinnelig løsning}

I dette kapittelet blir det gjort en analyser av nettstedet Sirkus Media har i dag, som tar utgangspunkt i teori og analyse som ble kartlagt i de foregående kapitlene. Målet med å analyseringen er å kartlegge eventuelle problemerområder som burde forbedres. Når utviklingen av det nye nettstedet er ferdig vil vi sammenligne de nye testresultatene med resultatene som blir presentert i dette kapittelet. Analysen har hovedvekt på brukeropplevelsen og noen tekniske aspekter.
