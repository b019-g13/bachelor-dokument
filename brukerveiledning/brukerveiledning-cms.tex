...
\vedlegg{Brukerveiledning for headless CMS}
...
\subsection{Om oss}
\q{Om oss}-komponenten henviser til \q{Om oss}-avsnittet på forsiden. Den kan likevel fint brukes til annen informasjon, når du ønsker dem samme layouten og grå bakgrunn.

Det som er spesielt med denne komponenten er at den er helt tom. Den fungerer som en slags wrapper for stylingen og gjør at teksten og annet innhold havner der den skal på siden. Derfor må du legge til \q{Om oss - barn} inni denne komponenten for å få samme layout som vist på bilde under.
...
\section{Ikon m. link}
For å få et ikon ved siden av en link, bruker du komponenten \q{Ikon m. link}. Dette er en komponent som typisk er inni en forelder.
...
For å legge til en link i menyen, trykk på menypunktet \q{Menyer}. Velg så menyen du ønsker å endre ved å trykke på det grønne ikonet. Da vil du komme til et nytt vindu. Her kan du velge blant alle linkene som eksisterer, og dra den over til \q{Menyens linker}. Trykk på \q{Oppdater} for å lagre.
